# ============================================================
#  Nematode Species Diversity — Analysis
#  Phylum: Nematoda | Response: species richness
...
# ============================================================
#  3. Filter Nematoda, compute species richness
# ============================================================
...
diversity <- nem %>%
  ...
  summarise(
    species_richness = n_distinct(tax_species, na.rm = TRUE),
    .groups = "drop"
  )
...
cat("\n========== Mann-Whitney U: species richness ~ location ==========\n")
mw <- wilcox.test(species_richness ~ location, data = diversity, exact = FALSE)
print(mw)
cat(sprintf("Median species richness — A: %.1f | D: %.1f\n",
            median(diversity$species_richness[diversity$location == "A"]),
            median(diversity$species_richness[diversity$location == "D"])))

cat("\n========== Kruskal-Wallis: species richness ~ week ==========\n")
kw <- kruskal.test(species_richness ~ as.factor(week), data = diversity)
print(kw)
...
y_max_div <- max(diversity$species_richness, na.rm = TRUE)

# p1 — Species richness by location (+ Mann-Whitney annotation)
p1 <- ggplot(diversity, aes(x = location, y = species_richness, fill = location)) +
  ...
  labs(title = "Nematode species richness by location",
       x = "Location", y = "Species richness") +
  theme_classic() +
...
p2 <- ggplot(site_avg, aes(x = urban_score, y = mean_richness,
  ...
  labs(title = "Mean nematode species richness vs Urban score",
       subtitle = paste0(
       ...
       x = "Urban score", y = "Mean species richness", colour = "Location") +
  theme_classic()
print(p2)
...
p3 <- ggplot(site_avg, aes(x = mean_pH, y = mean_richness,
  ...
  geom_smooth(aes(colour = location), method = "lm", se = TRUE) +
  geom_text(vjust = -0.8, size = 3) +
  labs(title = "Mean nematode species richness vs pH",
       subtitle = paste0(
         "A: rho = ", round(sp_ph_A$estimate, 2), ", p = ", round(sp_ph_A$p.value, 3),
         "   |   D: rho = ", round(sp_ph_D$estimate, 2), ", p = ", round(sp_ph_D$p.value, 3)
       ),
       x = "Mean pH", y = "Mean species richness", colour = "Location") +
  theme_classic()
print(p3)

# p4 — Richness over weeks (+ Kruskal-Wallis annotation)
p4 <- ggplot(diversity, aes(x = as.factor(week), y = species_richness, fill = location)) +
  ...
  annotate("text",
           ...
           size = 3.2, hjust = 0.5) +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.18))) +
  labs(title = "Nematode species richness across weeks",
       x = "Week", y = "Species richness", fill = "Location") +
  theme_classic()
print(p4)

# ============================================================
#  10. Boxplot summary statistics (median + IQR)
# ============================================================

cat("\n========== Boxplot stats: species richness by location ==========\n")
...
cat("\n========== Boxplot stats: species richness by week + location ==========\n")
